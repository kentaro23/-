...
export function FunctionMapDiagram() {
  return (
    <div className="texture-mesh rounded-3xl border border-brand-200 bg-white p-5 shadow-panel md:p-6">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="rounded-xl bg-white p-2 text-accent-500 ring-1 ring-brand-200">
  ...
  return (
    <div className="texture-mesh rounded-3xl border border-brand-200 bg-white p-5 shadow-panel md:p-6">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-stretch">
        <div className="rounded-2xl border border-brand-700 bg-brand-800 p-4">
          <p className="text-sm font-semibold text-white">意思決定（学会）</p>
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            {academicSide.map((item) => (
              <li key={item} className="rounded-lg bg-white/15 px-3 py-2 text-white">
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-2xl border border-accent-400 bg-accent-500 p-4">
          <p className="text-sm font-semibold text-white">実務（代行）</p>
          <ul className="mt-3 space-y-2 text-sm text-white">
            {officeSide.map((item) => (
              <li key={item} className="rounded-lg bg-white/15 px-3 py-2">
                {item}
              </li>
            ))}
          </ul>
